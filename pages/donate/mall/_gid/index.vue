<template>
    <div class="item_detail">
        ...
        <van-goods-action>
            <van-goods-action-button @click="toCart" icon="cart-o" :info="(cartInfo > 0) ? cartInfo : ''"/>
            <van-goods-action-button @click="addCollect" icon="star-o"
                                       :style="(goods.userHasCollect !== 0) ? 'color: #f7b444;':''"/>
            <van-goods-action-button @click="skuClick" text="加入购物车"/>
            <van-goods-action-button primary @click="skuClick" text="立即购买"/>
        </van-goods-action>
...
<script>
    ...
    import {goodsDetail, cartGoodsCount, collectAddOrDelete, cartAdd, cartFastAdd} from '~/assets/services/mallapi'

    import {setLocalStorage} from '~/assets/utils/local-storage'
