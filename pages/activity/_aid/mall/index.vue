<template>
    <div class="tab_home">
        <div class="tal_class_searchBox">
            <van-search placeholder="点击前往搜索" @click="$router.push({ name: 'activity-aid-mall-search',params:{aid:aid}})"/>
        ...
        <van-panel title="新品首发">
            <van-row gutter>
                <van-col span="12" v-for="(newGood ,index) in shopInfos.newGoodsList" :key="index">
                    <router-link :to="{ name: 'donate-mall-gid',params:{gid:newGood.id}}">
        ...
        <ActivityTabbar :page="2" :aid="$route.params.aid"/>
    </div>
</template>

<script>
    import ActivityTabbar from '~/components/activity/activityTabbar'
    import { getHome, goodsCategory, couponReceive } from '~/assets/services/mallapi';
    import loadMore from '~/assets/mixin/list-load-more';
    import scrollFixed from '~/assets/mixin/scroll-fixed';
        ...
        methods: {
            ...
            changeTabbar(o) {
                goodsCategory({ id: o.id}).then(res => {
                    let categoryId = res.data.currentCategory.id;
                    this.$router.push({
                        name: 'activity-aid-mall-list',
                        aid:this.aid,
